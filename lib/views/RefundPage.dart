  ...
  Future<void> _updateRefundStatus(int refundId, String status) async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        refunds = refunds.map((refund) {
          if (refund['id'] == refundId) {
            refund['status'] = status; // Update the status in the list
          }
          return refund;
        }).toList();
      });
                                  ...
                                  bool? confirmed = await showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        ...
                                        content: Text(
                                            "Are you sure to confirm this refund?"),
                                        actions: <Widget>[
                                          TextButton(
                                            child: Text("Cancel"),
                                            onPressed: () {
                                ...
                                onPressed: () async {
                                  bool? confirmed = await showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title:
                                            Text("Refund Reject Confirmation"),
                                        content: Text(
                                            "Are you sure to reject this refund?"),
                                        actions: <Widget>[
                                          TextButton(
                                            child: Text("Cancel"),
                                            onPressed: () {
